 ...
 *
 * **The chevron is drawn in CSS.** Several screens across these SPAs — the refinement panel here
 * among them — use literal `▸`/`▾` characters and render tofu boxes wherever the font has no glyph.
 * A rotated bordered corner costs the same and cannot fail to render, which is why the trees copied
 * in with this tab draw theirs that way and the older sites are a parked bug.
 *
