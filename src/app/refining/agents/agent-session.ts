...
import { TerminalSocket } from '../../project/agent/terminal-socket';
 ...
 *
 * `TerminalSocket` comes from `project/agent/`, where this SPA's refinement panel already put it. This
 * file is a copy from qits-spa-workspaces and that one is too, so a fresh copy beside it would be the
 * *same* file in the same repository twice — which is the one thing per-SPA duplication does not buy.
 * The reuse is safe because the socket takes a **URL**: it knows nothing about which daemon answers at
 * the far end, so pointing it at `/workspaces/container/{id}/terminal/commands/{id}` costs it nothing,
 * and the two daemons implement the same attach-and-replay contract line for line.
 */
