...
/**
 * The **workspace** daemon's command and coding-agent surface, hand-written from
 * `daemons/qits-workspace-daemon/docs/openapi.yml` and copied from qits-spa-workspaces.
 *
 * Not to be merged with `project-agent-api.ts` beside it, for the same reason
 * {@link ./workspace-daemon-api#WorkspaceDaemonApi} is not: that one drives the *projects* daemon's
 * one-container-per-project refinement agent, and this one drives a workspace's container. Two
 * daemons, two identifier types, two contracts that are free to diverge.
 *
