 ...
 *
 * The list is complete rather than trimmed to what is drawn today. Most of these currently fire
 * against readers that do not exist in this SPA yet — the channel is ahead of the page — and a counter
 * costs a signal, so declaring them all means a panel landing in a later phase wires to a topic that is
 * already ticking instead of adding one.
 *
 * Distinct from {@link ./project-events#PROJECT_TOPICS}, which is the *project's* channel and carries
 ...
 *
 * **Hint-and-refetch, not push-the-data.** Every frame is a topic name and nothing else. A panel reads
 * the counter for the topic it cares about inside an `effect` and re-issues its own ordinary REST
 * request when the number moves. That is why an idle workspace produces no traffic at all — polling has
 ...
 * connection open through intermediate proxies, and a newer service may invent a topic this build has
 * never heard of. Both fall through the same door.
 *
 * **Nothing here polls.** That is a rule and not a tendency: this page has a channel, so it must not
 * also put a traffic floor under a workspace nobody is changing.
 *
 * The service is application-scoped but singly-owned: the refining shell opens it for the workspace it
 * is showing and closes it on destroy. {@link connected} is what draws the quiet inline marker saying
 * the page is briefly behind — a stale-data notice, deliberately unlike the daemon's own disconnection,
 * which stops half the page working and gets a first-class state in the status strip.
 */
...
/**
 * A counter that moves when any of the given topics fires — the shape most panels actually want, since
 * "the workspace row is stale" is three topics rather than one.
 */
