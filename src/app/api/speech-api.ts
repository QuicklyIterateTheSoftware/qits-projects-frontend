 ...
 * this page talks to and it needs nothing special to reach: this SPA is served at `/projects/` behind
 * the same gateway that serves `/stt/`, so the session cookie rides a same-origin absolute path with
 * no CORS and no machine token — the same reason {@link QITS_API_BASE} is empty.
 *
 ...
 * reaches the same service from any SPA the gateway serves.
 *
