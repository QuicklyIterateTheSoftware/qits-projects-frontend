 ...
 * not exist.
 *
 * These are same-origin absolute paths on purpose. This SPA is served at `/projects/` and these calls
 * go to `/workspaces/api/…`, behind the one gateway that serves both — which is what carries the
 * session cookie across the boundary with no CORS and no machine token. The browser being the
 * integrator is the whole reason no server-side change was needed for refining, so the paths are worth
 * asserting exactly.
 */
describe('WorkspacesApi', () => {
  ...
  it('carries the repository in the create’s body, where the service reads it', async () => {
    ...
    request.flush({ workspace: { id: 12, workspaceId: 'refining-x', status: 'ACTIVE' } });
    await expect(created).resolves.toMatchObject({ id: 12, workspaceId: 'refining-x' });
  });
