 ...
 *
 * Copy-adapted from qits-spa-workspaces, the same deliberate per-SPA duplication as `ui/`.
 *
 * ## Why this exists instead of xterm.js
 *
 * xterm.js is a dependency, and a dependency is a lockfile change — which this workstream does not
 * make, because the lockfile's `integrity` is a fact about tarball bytes and a bump has to run
 * where the registry is reachable. So the refinement session ships with **this**: a grid, a cursor,
 * and the handful of sequences a full-screen redraw actually uses.
 *
 * It is also why this is not `ui/format`'s `renderTerminalText`, which the sign-in terminal uses.
 * That renderer honours carriage return and backspace and drops two-dimensional motion, which is
 * exactly right for git's line-oriented prompts and exactly wrong for a coding agent's TUI: an
 * agent repaints the whole screen, so dropped cursor addressing shows every frame stacked on the
 * last one.
 *
 ...
 * reporting and no wide-character accounting. Adding xterm.js later replaces this file and
 * {@link ./terminal-view#TerminalView}, and changes nothing else — {@link
 * ./terminal-socket#TerminalSocket} hands it bytes and reads back lines.
 *
