...
/**
 * One attachment to `WS /terminal/commands/{id}` on the project's agent daemon.
 *
 * Copy-adapted from qits-spa-workspaces. Every rule here was learned from a real failure there, and
 * the projects daemon's `CommandSockets` implements the same contract to the letter:
 *
 * **Opening re-attaches and replays the scrollback; closing only detaches.** The agent survives a
 * browser refresh, a collapsed panel and this object being thrown away — to actually stop it you
 * call `POST /commands/{id}/terminate`, which is a different verb in a different client on purpose.
 * That is what makes collapsing the refinement panel free.
 *
 * **A clean server close (1000) is final.** It means the command is gone: stop, and let the panel
 * offer a new session. Reconnecting into it would produce the daemon's yellow *"This command is no
 * longer running"* line on a loop.
 *
...
export class TerminalSocket {
   ...
   * Detach for good.
   *
   * The agent keeps running; this is called when the panel is collapsed or destroyed, or when the
   * command changes. Never when the tab is merely hidden — a detached terminal stops replaying and
   * a working agent then looks dead.
   */
  close(): void {
    this.stopped = true;
    this.clearTimer();
